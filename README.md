...
```
tin_man/
├── src/ Core library (models, RAMs, encoders, persistence)
...
Benchmarks are run with [Criterion](https://github.com/bheisler/criterion.rs) and measure training and classification/prediction throughput (operations per second) for all four models:
...
Integration tests exercising the public API of all four models — training, classification/prediction correctness, edge cases (untrained models, input size mismatches), and bleaching vs. binary mode consistency. Run with:
